...
async def test_critic_agent_llm_pass():
    ...
    scribe_out = ScribeOutput(
        ...
        recommendations=[
            Recommendation(symbol="RELIANCE", action="BUY", confidence=0.8, explanation="Bullish trend", stop_loss=2400.0)
        ],
        notes=[]
    )
...
async def test_critic_agent_llm_fail():
    ...
    llm.generate_structured = AsyncMock(return_value={
        "passed": False,
        "score": 0.5,
        "notes": ["Fails to mention VaR risk."]
    })
    ...
    scribe_out = ScribeOutput(
        ...
        agreement_level="CONSENSUS",
        recommendations=[
            Recommendation(symbol="RELIANCE", action="BUY", confidence=0.9, explanation="Strong trend")
        ],
        notes=[]
    )

    res = await agent.audit(scribe_out, sentinel_output={"portfolio_var_95": 0.12}, sage_outputs=[], iteration=1)
